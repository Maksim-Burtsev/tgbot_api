    ...
    def create_purchases(raw_purchases: List[str]) -> bool:
        ...
        for i in range(0, len(raw_purchases), 2):
            purchases_list.append(
                {
                    "name": raw_purchases[i],
                    "cost": raw_purchases[i + 1],
                    "date": today_date,
                }
            )
...
class Note:
    def get_notes(self, query: Optional[str]) -> List[Optional[str]]:
        """Return list of formatted notes on this query"""
        if query:
            raw_notes = self._get_notes(category=query)
            if not raw_notes:
                raw_notes = self._get_notes(name=query)
        else:
            raw_notes = self._get_notes()
    ...
    def delete_notes(self, query: str) -> bool:
        """Delete notes with query. First GET pk of notes with name and category, which contains query. Second is DELETE them"""
        if not query:
            return False

        with_name = [note["pk"] for note in self._get_notes(name=query) if note]
        with_category = [note["pk"] for note in self._get_notes(category=query) if note]
